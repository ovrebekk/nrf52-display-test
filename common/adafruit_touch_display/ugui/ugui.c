...
static UG_U16 m_assigned_index_text = 0, m_assigned_index_button = 0, 
              m_assigned_index_image = 0, m_assigned_index_checkbox = 0;
...
UG_S16 UG_Init(UG_GUI * g, UG_S16 x, UG_S16 y, const nrf_lcd_t * p_nrf_lcd)
{
    ...
    //MQ Add nrf_gfx initialization + auto-register hw accel functions
    APP_ERROR_CHECK(nrf_gfx_init(p_nrf_lcd));
    p_lcd = p_nrf_lcd;
...
void UG_FillFrame(UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_COLOR c)
{
    ...
    {
	if (((UG_RESULT(*)
	      (UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2,
	       UG_COLOR c)) gui->driver[DRIVER_FILL_FRAME].driver) (x1, y1, x2,
								    y2, c) == UG_RESULT_OK)
	    return;
    }
...
void UG_DrawLine(UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_COLOR c)
{
	...
	if (((UG_RESULT(*)
	      (UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2,
	       UG_COLOR c)) gui->driver[DRIVER_DRAW_LINE].driver) (x1, y1, x2,
								   y2, c) == UG_RESULT_OK)
	    return;
    }
...
void _UG_PutChar(char chr, UG_S16 x, UG_S16 y, UG_COLOR fc, UG_COLOR bc, const UG_FONT * font)
{
	...
	//(void(*)(UG_COLOR))
	push_pixel =
	    ((void *(*)(UG_S16, UG_S16, UG_S16, UG_S16)) gui->driver[DRIVER_FILL_AREA].driver) (x,
												y,
												x +
												actual_char_width
												- 1,
	...
	if (font->font_type == FONT_TYPE_1BPP)
	{
	    ...
	    _HW_draw_buffer(x, y, x + actual_char_width, y + font->char_height,
			    (void *) char_buf, actual_char_width * font->char_height * 2);
	}
	else if (font->font_type == FONT_TYPE_8BPP)
	{
...
/* -------------------------------------------------------------------------------- */
UG_RESULT
UG_ButtonCreate(UG_BUTTON * btn, UG_WINDOW * wnd, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye)
{
    UG_OBJECT *obj;
...
UG_RESULT UG_ButtonSetStyle(UG_BUTTON * btn, UG_U8 style)
{
    ...
    /* Select color scheme */
    btn->style &=
	~(BTN_STYLE_USE_ALTERNATE_COLORS | BTN_STYLE_TOGGLE_COLORS |
	...
	if (obj->state & OBJ_STATE_VISIBLE)
	{
		...
		{		/* 3D */
		    _UG_DrawObjectFrame(obj->a_abs.xs, obj->a_abs.ys,
					obj->a_abs.xe, obj->a_abs.ye,
					(btn->state & BTN_STATE_PRESSED) ? (UG_COLOR
									    *)
					pal_button_pressed : (UG_COLOR *) pal_button_released);
		}
		else
		{		/* 2D */
...
/* -------------------------------------------------------------------------------- */
UG_RESULT
UG_CheckboxCreate(UG_CHECKBOX * chb, UG_WINDOW * wnd, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye)
{
    UG_OBJECT *obj;
...
UG_RESULT UG_CheckboxSetStyle(UG_CHECKBOX * btn, UG_U8 style)
{
    ...
    /* Select color scheme */
    chk->style &=
	~(CHB_STYLE_USE_ALTERNATE_COLORS | CHB_STYLE_TOGGLE_COLORS |
...
void _UG_CheckboxUpdate(UG_WINDOW * wnd, UG_OBJECT * obj)
{
	...
	if (obj->state & OBJ_STATE_VISIBLE)
	{
	    ...
	    d2 = (chb->font->char_width <
		  chb->font->char_height) ? chb->font->char_height : chb->font->char_width;
...
//             txt.a.ye = obj->a_abs.ye-d;
		...
		UG_DrawLine(obj->a_abs.xs + d, obj->a_abs.ys + d,
			    obj->a_abs.xs + d2 + d - 1, obj->a_abs.ys + d2 + d - 1, chb->fc);
		...
		UG_DrawLine(obj->a_abs.xs + d2 + d - 1, obj->a_abs.ys + d,
			    obj->a_abs.xs + d, obj->a_abs.ys + d2 + d - 1, chb->fc);
		UG_DrawLine(obj->a_abs.xs + d2 + d - 2, obj->a_abs.ys + d,
			    obj->a_abs.xs + d, obj->a_abs.ys + d2 + d - 2, chb->fc);
	    }
	    else
	    {
		UG_DrawLine(obj->a_abs.xs + d + 1, obj->a_abs.ys + d,
		...
		UG_DrawLine(obj->a_abs.xs + d, obj->a_abs.ys + d,
			    obj->a_abs.xs + d2 + d - 1, obj->a_abs.ys + d2 + d - 1, chb->bc);
		UG_DrawLine(obj->a_abs.xs + d, obj->a_abs.ys + d + 1,
		...
		UG_DrawLine(obj->a_abs.xs + d2 + d - 1, obj->a_abs.ys + d,
			    obj->a_abs.xs + d, obj->a_abs.ys + d2 + d - 1, chb->bc);
		UG_DrawLine(obj->a_abs.xs + d2 + d - 2, obj->a_abs.ys + d,
			    obj->a_abs.xs + d, obj->a_abs.ys + d2 + d - 2, chb->bc);
	    }
	    /* Draw Checkbox frame */
		...
		{		/* 3D */
		    _UG_DrawObjectFrame(obj->a_abs.xs, obj->a_abs.ys,
					...
					(chb->state & CHB_STATE_PRESSED) ? (UG_COLOR
									    *)
					pal_checkbox_pressed : (UG_COLOR *) pal_checkbox_released);
		}
		else
		{		/* 2D */
...
/* -------------------------------------------------------------------------------- */
UG_RESULT
UG_TextboxCreate(UG_TEXTBOX * txb, UG_WINDOW * wnd, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye)
{
    UG_OBJECT *obj;
...
/* -------------------------------------------------------------------------------- */
UG_RESULT
UG_ImageCreate(UG_IMAGE * img, UG_WINDOW * wnd, UG_S16 xs, UG_S16 ys, UG_S16 xe, UG_S16 ye)
{
    UG_OBJECT *obj;
...
/* -------------------------------------------------------------------------------- */
/* -- NORDIC HW ACCELERATION FUNCTIONS                                           -- */
/* -------------------------------------------------------------------------------- */
#ifdef NORDIC_GUI
//MQ
UG_RESULT _HW_draw_line(UG_S16 x1, UG_S16 y1, UG_S16 x2, UG_S16 y2, UG_COLOR c)
{
    ...
    line.x_end = x2;
    line.y_end = y2;
    line.thickness = 1;		//MQ adjustable
